...
public class week8Test {
        ...
        void macro_executes_all_and_undo_reverts_entire_batch_in_reverse_order() {
                ...
                // Use different items so we can also assert reverse-order via a simple log if
                // needed
        ...
        @Test
        void adapter_converts_text_to_bytes_minimal() {
                LegacyThermalPrinter printer = new LegacyThermalPrinter();
                Printer adapter = new LegacyPrinterAdapter(printer);
                assertDoesNotThrow(() -> adapter.print("ABC"));
        }
